...
fn test_pool_init_can_only_be_called_once() {
    ...
    let result = client.try_initialize(&admin, &token.address, &share_token.address, &invoice_contract);
    assert_eq!(result.unwrap_err().unwrap(), PoolError::AlreadyInitialized.into());
}
...
fn test_pool_reinit_by_different_admin_rejected() {
    ...
    client.initialize(&admin1, &token.address, &share_token.address, &invoice_contract);
    
    // Attempt to reinitialize with admin2 should fail
    let result = client.try_initialize(&admin2, &token.address, &share_token.address, &invoice_contract);
    assert_eq!(result.unwrap_err().unwrap(), PoolError::AlreadyInitialized.into());
}
