...
export function assertEnvValid(): void {
  ...
  if (!valid) {
    ...
    // Do not throw during server-side rendering / static generation.
    // CI provides these env vars in the environments that actually need them (e.g. E2E),
    // while local dev may fill `.env.local`.
  }
}
